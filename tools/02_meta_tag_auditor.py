...
def audit_meta_tags(soup):
    ...
    if title and title.string:
        ...
        if title_len == 0:
            ...
            issues.append(('MEDIUM', f'Title too short ({title_len} chars). Recommend 50-60 chars.'))
        elif title_len > 60:
            issues.append(('MEDIUM', f'Title too long ({title_len} chars). Recommend 50-60 chars.'))
        else:
    ...
    if meta_desc and meta_desc.get('content'):
        ...
        present_tags.append(('Description', desc_content[:50] + '...' if desc_len > 50 else desc_content, f"{desc_len} chars"))

        if desc_len < 120:
            issues.append(('HIGH', f'Description too short ({desc_len} chars). Recommend 150-160 chars.'))
        elif desc_len > 160:
            issues.append(('LOW', f'Description slightly long ({desc_len} chars). Optimal: 150-160 chars.'))
        else:
    ...
    if og_count >= 2:
        present_tags.append(('Open Graph', f'✓ {og_count}/3 tags', 'Social sharing optimized'))
    elif og_count == 1:
        issues.append(('LOW', 'Incomplete Open Graph tags. Add og:title, og:description, og:image'))
    else:
        issues.append(('MEDIUM', 'Missing Open Graph tags (recommended for social sharing)'))
    ...
    if twitter_card:
        ...
        issues.append(('LOW', 'Missing Twitter Card tags (recommended for Twitter sharing)'))

    # Check schema.org
    schema_scripts = soup.find_all('script', attrs={'type': 'application/ld+json'})
    if schema_scripts:
        present_tags.append(('Schema.org', f'✓ {len(schema_scripts)} markup(s)', 'Structured data'))
    else:
    ...
    if h1_count == 1:
        ...
        issues.append(('MEDIUM', f'Multiple H1 tags ({h1_count}). Use only one H1 per page.'))
    ...
    if images:
        ...
        if alt_percentage >= 80:
            present_tags.append(('Images', f'✓ {len(images_with_alt)}/{len(images)} with alt', 'Good accessibility'))
        elif alt_percentage >= 50:
            issues.append(('LOW', f'{int(alt_percentage)}% of images have alt text. Aim for 100%.'))
        else:
            issues.append(('MEDIUM', f'Many images missing alt text ({len(images) - len(images_with_alt)} of {len(images)}).'))
